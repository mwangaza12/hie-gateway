...
// ─── PATCH /api/patients/:nupi/contact ───────────────────────────────────────
// Update a patient's phone number and/or email.
// Used when a patient was registered without contact details and needs them
// added before SMS-OTP consent can be sent.
...
router.patch("/:nupi/contact", requireFacility, async (req, res) => {
  try {
    ...
    if (!phoneNumber && !email) {
      return res.status(400).json({ success: false, error: "At least one of phoneNumber or email is required" });
    }

    const patient = chain.getPatient(nupi);
    if (!patient) {
      return res.status(404).json({ success: false, error: "Patient not on AfyaNet" });
    }
    ...
    await logAudit({
      event:       "patient_contact_updated",
      patientNupi: nupi,
      facilityId:  req.facilityId,
      success:     true,
      metadata:    { updatedFields: [phoneNumber && "phoneNumber", email && "email"].filter(Boolean) },
      ipAddress:   req.ip,
    });

    res.json({ success: true, nupi });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.post("/:nupi/consent/otp/request", requireFacility, async (req, res) => {
  try {
    ...
    if (!phone) {
      return res.status(400).json({ success: false, error: "No phone number registered for this patient" });
    }
